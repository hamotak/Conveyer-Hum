...
export function analyzeScenePlan(scenes: { text?: unknown }[]): ScenePlanHealth {
  ...
  const danglingScenes = texts.filter(endsOnDanglingWord).length;
  ...
  return {
    ok: !bad,
    issue: bad
      ? `Old micro-chunk scene plan detected: ${shortScenes}/${texts.length} beats are too short, ${danglingScenes} end mid-thought.`
      : null,
