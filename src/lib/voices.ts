...
export function addSavedVoice(input: {
  ...
  const provider = (input.provider || "elevenlabs").trim();
...
export function savedVoicesAsOptions(): VoiceOption[] {
  return listSavedVoices().map((v) => ({
    voiceId: v.voice_id,
    provider: (v.provider as VoiceOption["provider"]) || "elevenlabs",
    name: v.name,
    language: v.language,
    gender: v.gender,
...
export function seedCurrentVoice(): void {
  ...
  if (!voiceId) return;
  const provider = (getSetting("TTS_VOICE_PROVIDER") || "elevenlabs").trim();
  addSavedVoice({
    name: "My voice",
    voice_id: voiceId,
    provider,
