...
import { getDriveClient, findOrCreateFolder } from "./gdrive";
 ...
 *
 * Library clips live in Drive at  Conveyer Hum / Clips Library / <folder>
 * (e.g. "Pirates"). The hybrid pipeline (see pipeline.ts) uses these to fill the
...
/** Local cache root: ~/.conveyer-hum/library-cache/<folder>/ */
...
/** Resolve the Drive folder id for Conveyer Hum / Clips Library / <folder>. */
...
export async function listStockClips(folder: string): Promise<StockClip[]> {
  ...
  do {
    ...
    for (const f of res.data.files ?? []) {
      if (f.id && f.name) clips.push({ driveFileId: f.id, name: f.name });
    }
...
export async function cacheStockLibrary(
  ...
  let clips: StockClip[];
  try {
    clips = await listStockClips(folder);
  } catch (e) {
  ...
  if (clips.length === 0) {
    throw new Error(
      `The "${folder}" stock library is empty on Drive (Conveyer Hum / Clips Library / ${folder}). Add clips first.`
    );
  }
