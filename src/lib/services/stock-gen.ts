...
import { findOrCreateFolder, uploadFile } from "./gdrive";
...
async function resolveDriveStockFolder(folder: string): Promise<{ id: string; link: string }> {
  const drive = await findOrCreateFolder("Conveyer Hum");
  const lib = await findOrCreateFolder("Clips Library", drive);
  const folderId = await findOrCreateFolder(folder, lib);
  return { id: folderId, link: `https://drive.google.com/drive/folders/${folderId}` };
}
...
export function startStockGeneration(opts: {
  ...
  const status: StockGenStatus = {
    ...
    promptMode: opts.promptMode || (exactPrompts.length ? "mixed" : "brief"),
    exactPrompts,
    aiPrompts: [],
    promptSource: exactPrompts.length ? "mixed" : "ai",
  ...
  void (async () => {
    try {
      ...
      const driveFolder = await withTimeout("Drive folder check", resolveDriveStockFolder(folder), DRIVE_PRECHECK_TIMEOUT_MS);
  ...
  void (async () => {
    ...
    try {
      ...
      const driveFolder = status.driveFolderId
        ...
        : await withTimeout("Drive folder check", resolveDriveStockFolder(status.folder), DRIVE_PRECHECK_TIMEOUT_MS);
      status.driveFolderId = driveFolder.id;
      status.driveFolderLink = driveFolder.link;
      ...
      step.lastProgressAt = Date.now();
      remember(status);
      const driveName = `gen_${Date.now().toString(36)}_${String(step.index).padStart(3, "0")}.mp4`;
      const driveId = await uploadFile(videoPath, driveFolder.id, { name: driveName, mimeType: "video/mp4" });
      step.uploadStatus = "done";
      step.uploadFinishedAt = Date.now();
      step.driveFileId = driveId;
