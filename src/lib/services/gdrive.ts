 ...
 * settings, they're returned as-is. Otherwise creates `Conveyer Hum/Final Videos`
 * and `Conveyer Hum/Clips Library` in the user's Drive root and persists the IDs.
...
export async function ensureTopLevelFolders(): Promise<{
  ...
  if (!finalId || !clipsId) {
    const rootFolder = await findOrCreateFolder("Conveyer Hum");
    if (!finalId) {
